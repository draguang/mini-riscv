...
void call_function(word_t pc,vaddr_t snpc);
void ret_function(word_t pc,vaddr_t snpc);
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, uint32_t shamt = BITS(imm,4,0);R(rd) = (SEXT(BITS(src1, 31, 31), 1) << (32 - shamt)) | (src1 >> shamt));//这个好像有问题
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", cssrw  , I, R(rd) = CSR(imm);CSR(imm) = src1);
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, bool success; s->dnpc = isa_raise_intr(isa_reg_str2val("a7", &success), s->pc);printf("a7:%x\n",isa_reg_str2val("a5", &success)));
